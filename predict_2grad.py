...
def cal_xs(img, xs):
    data = np.array(img[:, int(192)])
    data = prctile_norm(data)
    real_xs = xs / data[int(104)]
    return real_xs

def initial():
    im = Input(shape=(8, 1, 2))
    out = Input(shape=(8, 1, 2))
    ...
    g.load_weights("trained_models/logics_pattern_fft_35-65_8_256cen_2pic_mean_MT_10000-SIM_0.3um/weights.best.h5")


def cut_fft(img, depth):
    # print(img.shape)
    #floor(400 * (2 * k - 1) / grad) - pic_size / 2 + 1: floor(400 * (2 * k - 1) / grad) + pic_size / 2, center(
      #  k) - pic_size / 2: center(k) + pic_size / 2 - 1
    # img = prctile_norm_img(img)
    im = img[int(400 * (2 * (depth+1) - 1) / 2 - 384 / 2 + 1):int(400 * (2 * (depth+1) - 1) / 2 + 384/2+1), int(256 - 384/2-1):int(256 + 384/2-1)]
...
def predict(onfocus, out):
    ...
    for j in range(0, 2):
        ...
        rz.append(img[104]*0.95)
        img = img[100:108] * a *0.95
        img = img[:, np.newaxis]
        cur_img.append(img)
...
if process_single_img:
    ...
    rz2=[]
    rz3=[]
    data_path='data/test/mitochondrion/stack/cell2G2Dz0p3um_ch0_stack0001_488nm_0003524msec_0002913748msecAbs00'
    star=35
    end=65
    for i in range(star, end):
        all_img = []
        onfocus = imageio.imread(data_path + str(50) + '.tif').astype(np.float)
        all_img.append(onfocus)
    ...
    ax.scatter(np.linspace(0,len(depth),len(depth)),depth,s=60, c='hotpink', marker='.',label='predict')
    plt.legend()
    plt.show()
